...
export const generateBnw = async (
  profilePicture: Buffer,
  text: string,
  author?: string
) => {
  const textCanvas = await generateText(750, 550, text, author)

  const background = sharp(imagePath)
  const backgroundMetric = await background.metadata()
  const authorImage = sharp(profilePicture).resize({
  ...
  return await sharp({
    ...
    .composite([
      ...
      },
      {
        ...
        left:
          350 +
          Math.ceil((backgroundMetric.width! - textCanvas.width - 350) / 2)
      }
    ])
    .jpeg({
      quality: 75
